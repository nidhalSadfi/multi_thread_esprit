...
def worker_ping(thread_id):
    left_fork = forks[thread_id]
    right_fork = forks[(thread_id + 1) % num_philosophers]
    ...
    while True:
        ...
        with left_fork:  # Acquire left fork
            with right_fork:  # Acquire right fork
...
# Number of philosophers (threads) and forks (semaphores)
num_philosophers = 5
forks = [threading.Semaphore(1) for _ in range(num_philosophers)]

# Initialize thread status list
thread_status = ["Idle"] * num_philosophers

# Create and start threads for pinging
ping_threads = []
for i in range(num_philosophers):
    t = threading.Thread(target=worker_ping, args=(i,))
    t.daemon = True
    t.start()
...
port_threads = []
for i in range(num_philosophers):
    t = threading.Thread(target=worker_port, args=(i,))
    t.daemon = True
    t.start()
